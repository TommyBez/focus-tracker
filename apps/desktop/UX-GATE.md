...
| F14 | Global Quick Focus shortcut | With another app frontmost, open and refocus the single Quick Focus window; change the combination and prove the old one stops while the new one works. Relaunch while the custom combination remains enabled and verify that it is restored and works while the old shortcut remains inactive. Then disable and relaunch to verify that the disabled state persists. A rejected combination must leave the saved shortcut unchanged and keep its prior runtime state |
