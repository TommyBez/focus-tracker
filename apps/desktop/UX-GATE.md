...
| F14 | Global Quick Focus shortcut | With another app frontmost, open and refocus the single Quick Focus window; change the combination and prove the old one stops while the new one works. On a non-US input layout, verify that each letter is described and activated as its physical US key position. Confirm Command + Shift + Space remains reserved for timer transport. Relaunch while the custom combination remains enabled and verify that it is restored and works while the old shortcut remains inactive. Then disable, change the saved combination while off, and relaunch to verify both values persist. A rejected combination must leave the saved shortcut unchanged and keep its prior runtime state |
